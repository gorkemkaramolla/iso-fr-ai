...
# ISO AI v4 with face recognition library and OpenCV. 
# Adjust tolerance to make comparison stricter
...
# Prepare flattened list of encodings and corresponding labels
all_face_encodings = []
all_face_names = []
for name, encodings in person_face_encodings.items():
    all_face_encodings.extend(encodings)
    all_face_names.extend([name] * len(encodings))
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if face_encoding:
            ...
            # Adjust tolerance value as needed
            tolerance = 0.25  # Lower this value to make comparison stricter
            ...
            if best_match_index is not None and distances[best_match_index] <= tolerance:
                ...
                face_names.append(f'{name} ({confidence:.2%})')
            else:
