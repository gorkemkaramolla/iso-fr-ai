...
from camera_storage import read_camera_urls, write_camera_urls
...
def _generate_response_frame(camera, stream_id):
...
def add_camera_url():
    ...
    write_camera_urls(camera_urls)

    return jsonify({'message': 'Camera URL added successfully'}), 200

@app.route('/camera-urls', methods=['GET'])
def get_camera_urls():
    return jsonify(camera_urls), 200

@app.route('/camera-url/<label>', methods=['DELETE'])
...
def ip_camera_stream(stream_id):
    ...
    if camera_label not in camera_urls:
        return jsonify({'error': 'Camera label not found'}), 404
    
    camera = camera_urls[camera_label] + quality
    return Response(_generate_response_frame(camera, stream_id), mimetype='multipart/x-mixed-replace; boundary=frame')
...
def _open_local_camera(cam_id):
    ...
    while True:
        ...
        yield (b'--frame\r\n' b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n')
    cap.release()
    # del cap  # Remove capture object when done

if __name__ == '__main__':
    app.run(port=5002, threaded=True, debug=True)
# @app.route('/stop-stream/<int:stream_id>', methods=['POST'])
# def stop_stream(stream_id):
#     print("STOP STREAM: " + str(stream_id))
#     with video_capture_locks:  # Ensure thread safety
#         if stream_id in video_captures:
#             cap = video_captures[stream_id]
#             cap.release()
#             del video_captures[stream_id]
