...
def draw_bounding_boxes(image, bboxes, labels, sims, threshold=0.4, font_scale=0.5):
    for bbox, label, sim in zip(bboxes, labels, sims):
        x1, y1, x2, y2 = map(int, bbox[:4])
        cv2.rectangle(image, (x1, y1), (x2, y2), (0, 255, 0), 2)
        if sim < threshold or label == "Unknown":
            # Label as "Unknown" if below threshold or already labeled as "Unknown"
            cv2.putText(image, "Unknown", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), 1)
        else:
            # Otherwise, label with the best match name and similarity score
            label_with_sim = f"{label} ({sim:.2f})"
            cv2.putText(image, label_with_sim, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), 1)
    return image
def draw_bounding_boxes(image, bboxes, labels, sims, threshold=0.4, font_scale=0.5):
    for bbox, label, sim in zip(bboxes, labels, sims):
        x1, y1, x2, y2 = map(int, bbox[:4])
        ...
        if sim < threshold or label == "Unknown":
            # Label as "Unknown" if below threshold or already labeled as "Unknown"
            cv2.putText(image, "Unknown", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 0, 255), 2)
        else:
            # Otherwise, label with the best match name and similarity score
            label_with_sim = f"{label} ({sim:.2f})"
            cv2.putText(image, label_with_sim, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 0), 2)
    return image


def gen_frames():
    cap = cv2.VideoCapture(0)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 256)  # Set the width to 256
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 144)  # Set the height to 144

    database = create_face_database(rec, detector, '../face-images/')  # Initialize database
    ...
    try:
        while True:
            ...
            if not success:
                ...
                bboxes, labels, sims = func(frame, database)  # Detect faces, labels, and similarity scores
                frame_with_boxes = draw_bounding_boxes(frame, bboxes, labels, sims)  # Draw boxes and labels with similarity
                ret, buffer = cv2.imencode('.jpg', frame_with_boxes)
                frame_with_boxes_encoded = buffer.tobytes()
                yield (b'--frame\r\n'
                       b'Content-Type: image/jpeg\r\n\r\n' + frame_with_boxes_encoded + b'\r\n')
    finally:
        cap.release()  # Release the capture object after streaming ends
